...
func (h *Handler) ListStandardizedProducts(c *gin.Context) {
	...
	if err != nil {
		...
		h.logger.Errorf("error listing products: %s", err.Error())
		return
	}
...
func (h *Handler) ShowProductInfo(c *gin.Context) {
	...
	if err != nil {
		errors.RenderError(c, http.StatusInternalServerError, "shop/categories", "get", err)
		h.logger.Errorf("error getting product id: %s", err.Error())
		return
	}
	...
	if err != nil {
		errors.RenderError(c, http.StatusInternalServerError, "shop/categories", "get", err)
		h.logger.Errorf("error getting product by id: %s", err.Error())
		return
	}
